...
class SimpleHTTPRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        accuracy = np.max(prediction_values)

        # Convertir el índice de la predicción a su correspondiente etiqueta
        prediction_label = class_names[prediction]

        print("Prediccion final: " + prediction_label)

        #Regresar respuesta a la peticion HTTP
        response_message = f"Prediccion: {prediction_label}, Accuracy: {accuracy:.2f}"
